...
import { validateFlutterwaveWebhook } from '@/lib/payment';
...
export async function POST(request: Request) {
    try {
        ...
        const isValid = validateFlutterwaveWebhook(signature, payload);
        if (!isValid) {
            return NextResponse.json(
        ...
        const serviceCharge = await ServiceCharge.findOne({
            'payments.reference': payload.data.tx_ref
        });
        ...
        const payment = serviceCharge.payments.find((p: { reference: string }) => p.reference === payload.data.tx_ref);
        if (!payment) {
            return NextResponse.json(
                { error: 'Payment not found' },
                { status: 404 }
